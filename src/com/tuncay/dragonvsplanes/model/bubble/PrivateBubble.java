...
import com.tuncay.dragonvsplanes.model.weapon.KnifeWeapon;

public class PrivateBubble extends Bubble{

	private static final String TAG = PrivateBubble.class.getSimpleName();

	public PrivateBubble(Context context, int x, int y, Bubble hedefBubble){
		setGoodOrEvil(GoodOrEvil.Evil);
		this.setId(nextId);
		nextId += 1;
		this.bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.aircraft11);
		this.setX(x);
		this.setY(y);
		...
		this.setWeapon(new KnifeWeapon());	
		this.setBody(new SmallBody());
		this.setBoss(false);
		this.setRowBitmap(4);
		
		Log.d(TAG, "PrivateBubble is being created");		
	}
