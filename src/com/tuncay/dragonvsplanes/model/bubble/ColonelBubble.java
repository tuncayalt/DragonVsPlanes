...
public class ColonelBubble extends Bubble{
	private static final String TAG = ColonelBubble.class.getSimpleName();

	public ColonelBubble(Context context, int x, int y, Bubble hedefBubble){
		setGoodOrEvil(GoodOrEvil.Evil);
		this.setId(nextId);
		nextId += 1;
		this.bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.aircraft32);
		this.setX(x);
		this.setY(y);
		...
		Log.d(TAG, "ColonelBubble is being created");		
	}	
