...
public class CorporalBubble extends Bubble{
	
	private static final String TAG = CorporalBubble.class.getSimpleName();

	public CorporalBubble(Context context, int x, int y, Bubble hedefBubble){
		setGoodOrEvil(GoodOrEvil.Evil);
		this.setId(nextId);
		nextId += 1;
		this.bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.aircraft12);
		this.setX(x);
		this.setY(y);
		...
		Log.d(TAG, "CorporalBubble is being created");		
	}	
